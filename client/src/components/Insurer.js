...
const Insurer = ({mediChain, account, ethValue}) => {
    ...
    const approveClaim = async (e, claim) => {
        ...
        mediChain.methods.approveClaimsByInsurer(claim.id).send({from: account, value: Web3.utils.toWei(floatValue.toString(), 'Ether')}).on('transactionHash', (hash) => {
            return window.location.href = '/login'
        })
    }
                ...
                <div className='box'>
                    ...
                    <Form>
                        ...
                        <Form.Group className='mb-3'>
                            ...
                            <Form.Control required type="number" value={polCoverValue} onChange={(e) => setPolCoverValue(e.target.value)} placeholder='Enter the cover value in PKR'></Form.Control>
                        </Form.Group>
                        <Form.Group className='mb-3'>
                            <Form.Label>Yearly Premium: </Form.Label>
                            <Form.Control required type="number" value={polPremium} onChange={(e) => setPolPremium(e.target.value)} placeholder='Enter the annual premium in PKR'></Form.Control>
                        </Form.Group>
                        <Form.Group className='mb-3'>
                            ...
                            { policyList.length > 0 ?
                                policyList.map((pol, idx) => {
                                    ...
                                    <tr key={idx+1}>
                                        <td>{idx+1}</td>
                                        <td>{pol.name}</td>
                                        <td>PKR {pol.coverValue}</td>
                                        <td>PKR {pol.premium}/year</td>
