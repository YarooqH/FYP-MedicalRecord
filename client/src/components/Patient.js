...
import { chatbotOptions } from '../utils';
import MedllamaIcon from '../assets/logo2.jpg'
...
const Patient = ({ mediChain, account, ethValue }) => {
  ...
  const purchasePolicy = async (e) => {
    ...
    let roundedVal = value.toFixed(8);
    let floatValue = parseFloat(roundedVal);
    mediChain.methods.buyPolicy(parseInt(policyList[buyPolicyIndex].id)).send({ from: account, value: Web3.utils.toWei(floatValue.toString(), 'Ether') }).on('transactionHash', (hash) => {
      return window.location.href = '/login'
    })
  }
  ...
  const settlePayment = async (e, transaction) => {
    ...
    // const truncatedValue = value.toFixed(1); // Truncate to 18 decimals
    // const weiValue = web3.utils.toWei(truncatedValue);
  ...
  const handleShowRecordModal = async () => {
    var record = {}
    await fetch(`http://127.0.0.1:8080/ipfs/${patient.record}`)
      .then(res => res.json())
  ...
  return (
    <div>
      {patient ?
        <>
          <ChatBot options={{chatHistory: {storageKey: "medllama_chat"}, ...chatbotOptions, theme: {primaryColor: '#008080', showFooter: false}, chatButton: "https://img.freepik.com/premium-vector/cute-business-llama-icon-illustration-alpaca-mascot-cartoon-character-animal-icon-concept-isolated_138676-989.jpg"}} flow={flow} />
          <div className='box'>
            ...
            {patient.policyActive && insurer
              ...
              <>
                ...
                <Form>
                  ...
                  <Form.Group>
                    <Form.Label>Remaining Cover Value: PKR {patient.policy.coverValue}</Form.Label>
                  </Form.Group>
                  <Form.Group>
                    <Form.Label>Premium: PKR {patient.policy.premium}/year</Form.Label>
                  </Form.Group>
                </Form>
              </>
              :
                ...
                <Form onSubmit={purchasePolicy}>
                  ...
                  {policyList[buyPolicyIndex]
                    ? <div>
                      ...
                      <p>Cover Value: PKR {policyList[buyPolicyIndex].coverValue}</p>
                      <p>Premium: PKR {policyList[buyPolicyIndex].premium}/year</p>
                    </div>
                    : <></>
                  }
                ...
                <Table id='records' striped bordered hover size="sm">
                  <thead>
                    <tr>
                      ...
                      <th style={{color: 'black'}} >Doctor&nbsp;Email</th>
                      <th style={{color: 'black'}} >Date</th>
                      <th style={{color: 'black'}} >Disease</th>
                  ...
                  <tbody>
                    {patientRecord.treatments.length > 0 ?
                      patientRecord.treatments.map((treatment, idx) => {
                        return (
                          <tr style={{backgroundColor: '#1c1c1c'}} key={idx + 1}>
                            <td>{idx + 1}</td>
                            ...
                            <td>
                              {treatment.prescription ?
                                <Link to={`http://127.0.0.1:8080/ipfs/${treatment.prescription}`} target="_blank"><Button variant="coolColor">View</Button></Link>
